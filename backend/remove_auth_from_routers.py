#!/usr/bin/env python3
"""
Script pour supprimer automatiquement l'authentification de tous les routeurs backend.
Remplace get_current_user et require_admin par des valeurs par défaut.
"""
import os
import re
...
def remove_auth_from_file(file_path: Path):
    ...
    try:
        ...
        content = re.sub(
            r'from app\.utils\.permissions import (get_current_user|require_admin|get_current_user, require_admin|require_admin, get_current_user)\n',
            '# Authentification supprimée - toutes les routes sont publiques\n',
            content
        )
        content = re.sub(
            r', get_current_user|get_current_user, |get_current_user|require_admin, |, require_admin|require_admin',
            '',
            content
        )
        ...
        content = re.sub(
            r'if\s+attempt\.get\("user_id"\)\s+!=\s+current_user\["id"\]:.*?raise HTTPException\([^)]+\)',
